...
##+++++BLIND mu = 0+++++
#python looTcombine.py ${BOSON}_${CHANNEL}_${YEAR}_workspace.root ${rMin} ${rMax} 0
#
...
#+++++BLIND mu = 1+++++
python looTcombine.py ${BOSON}_${CHANNEL}_${YEAR}_workspace.root ${rMin} ${rMax} 1
...
python check_sigle_nuisance.py -ws ${BOSON}_${CHANNEL}_${YEAR}_workspace.root -rMin ${rMin} -rMax ${rMax} -o . -expSig 1
...
#+++++UNBLIND+++++
#initial fit of mu
combineTool.py -M Impacts -d ${BOSON}_${CHANNEL}_${YEAR}_workspace.root -m 200 --rMin ${rMin} --rMax ${rMax} --robustFit 1 --doInitialFit

#fit separately every nuisance
combineTool.py -M Impacts -d ${BOSON}_${CHANNEL}_${YEAR}_workspace.root -m 200 --rMin ${rMin} --rMax ${rMax} --robustFit 1 --doFits
...
#plot likelyhood scan for each syst
python check_sigle_nuisance.py -ws ${BOSON}_${CHANNEL}_${YEAR}_workspace.root -rMin ${rMin} -rMax ${rMax} -o .
timeout --preserve-status --kill-after=60 60 python plot2Dscan_params.py -i higgsCombinename.MultiDimFit.mH120.root -x r --x-label mu --outputdir .
#python plot2Dscan_params.py -i higgsCombinename.MultiDimFit.mH120.root -x r --x-label mu --outputdir .
